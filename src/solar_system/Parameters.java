...
public class Parameters {
    ...
    private static final double SHIP_INITIAL_HEIGHT = 1500 * KM;
    private static final double SHIP_INITIAL_BASE_SPEED = 7.12 * KM;
    private static final double SHIP_INITIAL_ADDED_SPEED = -3 * KM;
    ...
    private static Vector initialShipPosition() {
        double angle = EARTH_INITIAL_Y / new Vector(EARTH_INITIAL_X, EARTH_INITIAL_Y).norm();
        double xPosition = EARTH_INITIAL_X + EARTH_RADIUS + Math.cos(angle) * SHIP_INITIAL_HEIGHT;
        double yPosition = EARTH_INITIAL_Y + EARTH_RADIUS + Math.sin(angle) * SHIP_INITIAL_HEIGHT;
        return new Vector(xPosition, yPosition);
    }

    private static Vector initialShipSpeed() {
        double earthModuleSpeed = new Vector(EARTH_INITIAL_SPEED_X, EARTH_INITIAL_SPEED_Y).norm();
        double angle = EARTH_INITIAL_SPEED_Y / earthModuleSpeed;
        double xSpeed = Math.sin(angle) * (earthModuleSpeed + SHIP_INITIAL_BASE_SPEED + SHIP_INITIAL_ADDED_SPEED);
        double ySpeed = Math.cos(angle) * (earthModuleSpeed + SHIP_INITIAL_BASE_SPEED + SHIP_INITIAL_ADDED_SPEED);
